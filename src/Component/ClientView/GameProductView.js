...
export default function ConsoleProductView(props){
    ...
    var item = props.history.location.state.gameproduct
   ...
   const handleDateDifference=(event,rentamt)=>{
      ...
      //alert("End Date"+endDate)
       ...
       //alert(JSON.stringify(item))
       dispatch({type:'ADD_CART', payload:[item.gameid,item]})
  ...
  const fetchProductPictures=async()=>{
    var body={gameid:item.gameid}
    var result=await postData('gamespicture/displayallgamespictures',body)
...
///////////////////////////////////////Tabs///////////////////////////////////////////////
   ...
   const showTabs=(Description)=>(
    <Tabs style={{padding:20}}>
      ...
      <TabPanel>
        <h2 style={{fontWeight:500,fontSize:16}}>{Description}</h2>
      </TabPanel>
  ...
  const handleQtyChange=(value,item)=>{
    ...
    if(value==0){
      dispatch({type:'REMOVE_CART', payload:[item.gameid]})
    }
    else{
  ...
  setPageRender(!pageRender)
   ...
   const ProductDetails=()=>{
       ...
       var v=cart[item.gameid]||0
      var qty=0
       if(v!=0)
       { qty=cart[item.gameid].qtydemand }
       return(<div>
